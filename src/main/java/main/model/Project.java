...
public class Project implements Comparable<Project> {
            ...
            public void run() {
                {
                    ...
                    int thisSecond = Calendar.getInstance().get(Calendar.SECOND); // nur damit nicht zu schnell gez�hlt
                    // wird
    ...
    private boolean saveAsFile(String ext) {
        ...
        lastDateToSave.add(Calendar.MONTH, -3); // Saving the last 3
        // months
        try {
        ...
        for (int t = 0; t < getWorkamountVec().size(); t++) {
            ...
            if (amount.getSecondsThatDay() == 0) {
                continue; // do not write entries with 0 seconds
            }
    ...
    private synchronized void load() {
        ...
        while (true) {
            try {
                ...
                st.nextToken(); // "Seconds" in Configfile �berlesen
                ...
                try {
                    st.nextToken(); // "Time:"
                    st.nextToken(); // the human readable time
                ...
                try {
                    ...
                    JOptionPane
                            .showMessageDialog(null,
                                    ...
                                    + getProjektName()
                                    + " An error has occurred. The saved times may not be correct!\nPlease correct the error and restart the program",
            ...
            // check if the workamount just read is the one for today
        ...
        try {
            ...
            // if there is no workload for the current day, create a
            // and remember
