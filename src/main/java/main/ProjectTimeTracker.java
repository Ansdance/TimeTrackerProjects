...
public class ProjectTimeTracker {
    ...
    public static final int BACKUP_DELAY_SECS = 300; // Save every 5 minutes
        ...
        for (final Project project : projects) {
            if (project.getProjektName().equals(name) && project != projectToBeNamed) {
                JOptionPane.showMessageDialog(window,
                        "Project " + name + " already exists for project number " + project.getProjektNumber() + ".");
                isAvailable = false;
            }

            if (number.length() > 0 && project.getProjektNumber().equals(number) && project != projectToBeNamed) {
                JOptionPane.showMessageDialog(window,
                        "Projectnummer " + number + "  already taken for project " + project.getProjektName() + ".");
                isAvailable = false;
            }
        }
        ...
        for (int t = 0; t < plist.length; t++) {
            if (plist[t].endsWith(".cpr")) {
                ...
                final Project p = new Project(plist[t].substring(0, plist[t].lastIndexOf(".")),
                        null /* Projectnummer */, ProjectTimeTracker.rootDir);
    ...
    private void loadSettings() {
        ...
        try {
            ...
            for (final GraphicsDevice device : GraphicsEnvironment.getLocalGraphicsEnvironment().getScreenDevices()) {
                maxHeight = device.getDisplayMode().getHeight(); // adding up the heights doesn't make sense. the
                // Monitors are hopefully next to each other...
    ...
    public void saveAllProjectsAndSettings() {
        for (final Project p : projects) {
            ...
            // Automatically delete projects for those in the
            // no entries have accumulated in the last 3 months
    ...
    public void exportReportInNotepad() {
        try {
            ...
            Runtime.getRuntime().exec("notepad " + ProjectTimeTracker.rootDir + "times.txt");
